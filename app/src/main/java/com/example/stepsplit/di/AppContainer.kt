...
import com.example.stepsplit.data.stepsource.HealthConnectStepSource
 ...
 * lifetime.
 *
 * Two entirely independent feature areas share only this container, the physical Room database,
 * and app navigation: automatic step counting/classification (this file's [stepRepository] and
 * everything it depends on) and manually started GPS trip recording ([tripRepository] and
 * everything below it). Neither reads, writes, or otherwise influences the other - see each
...
class AppContainer(context: Context) {
    ...
    // ---- Automatic step counting (see data.repository.StepRepository) - Health Connect is the
    // sole production step source; see HealthConnectStepSource's own doc comment for why this app
    // does not combine or merge several simultaneously active acquisition paths. ----

    // Diagnostics for production acquisition health (subscribe/read outcomes, whether any sample
    // has ever been observed) - a concern separate from StepSourceAvailability and SyncFailure,
    // see StepSourceHealthStore's own doc comment. Held here (not just inside
    // HealthConnectStepSource) so the UI layer can observe it directly for the "waiting for first
    ...
    // the same pattern as repositoryScope/tripRecordingScope below.
    private val healthEventScope: CoroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.Default)

    // HealthConnectStepSource must never await real diagnostic persistence directly on its own
    // acquisition coroutine (a wedged DataStore write would then block a real read) - see
    // AsyncStepSourceHealthRecorder's own doc comment. stepSourceHealthStore above is still
    // exposed directly (not through this wrapper) for the UI layer's own snapshot access, which
    // has no such latency constraint.
    ...
    // The app's single production StepSource - see HealthConnectStepSource's own doc comment for
    // why its id reuses the old Local Recording source's own id.
    val stepSource: StepSource = HealthConnectStepSource(
        gateway = PlatformHealthConnectGateway(context),
        healthStore = asyncHealthRecorder,
        clock = clock,
    )

    /**
     * Debug-only device/Health-Connect environment snapshot for the Settings debug panel - null in
     * release builds (never collected there at all) and null if collection itself fails for any
     * reason. Lazily computed on first access rather than eagerly here in the constructor:
     * [AppContainer] is built synchronously on the main thread from
     * [com.example.stepsplit.StepSplitApplication.onCreate], so anything computed unconditionally
     * at this point sits directly on the app's cold-start path. Nothing touches this property
     * until a debug build's Settings screen is actually opened (see `SettingsViewModel`), so app
     * startup itself never pays for it at all.
     */
    ...
    // ---- Manual GPS Trip Recording (see data.trip.TripRepository) - entirely separate from the
    // step counting/classification pipeline above; nothing here is read or written by
    // StepRepository, and vice versa. GPS trip data is never mixed into step totals or
    // classification, per the product requirement that these two features stay logically
    // separate: no shared repositories, DAOs, services, permissions, or domain state. ----
